...
(defun draw-board (&optional numbers-p)
  "Displays the board. If numbers-p is non-nil, displays numbers in vacant spaces."
  (format t "     |     |     ~%")
  (if numbers-p
      (format t "   ~a |   ~a |   ~a  ~%"
...
(defun play (&optional switch-p)
  "If switch-p is true, switches the active player. Plays a move; Checks if the move is a win or a stalemate. If neither is true, recursively calls itself. If either is true, prompts for a new game."
...
(defun read-choice ()
  "Displays the board and prompts the player to enter a choice, or to see their options, then reads in and returns their choice."
  (draw-board t)
...
(defun check-choice (choice)
  "Checks to ensure the user input is valid, i.e. an integer value between 1 and 9, inclusive."
...
(defun select (cell)
  "Checks if the cell has not been selected (contains a number). If so, places the players marker in the cell. Otherwise, prompts the player to try again."
...
(defun invalid-selection ()
  "Warns the player if they choose a spot that has been taken, and repeats the entry process."
  (format t "That spot is taken. Please choose another spot.~%~%")
  (force-output nil)
  (check-choice (read-choice)))

(defun check-for-win-p ()
  "Checks to see if the last move played was a winner. Compares all possible winning combinations on the board.."
  (or
   (dolist (x '((1 2 3)
		(1 4 7)
		(1 5 9)
...
(defun is-line-p (3-cells)
  "Tests to see if the three cells fed to it as arguments contain identical values. Returns t or nil."
   (equal
...
(defun stalemate-p ()
  "Checks to see if there is a stalemate, i.e. none of the board's cells contain a number. Returns t or nil."
